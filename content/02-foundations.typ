...
- *Bitübertragungsschicht (Layer 1, Physical Layer):* Definiert die physikalische Übertragung von Bitfolgen über ein Medium, u. a. Spannungspegel, Steckerbelegung und Signalkodierung. Sie wird in dieser Arbeit vor allem im Zusammenhang mit dem #acr("PHY")-Baustein relevant (siehe @hardware-basics).

- *Sicherungsschicht (Layer 2, Data Link Layer):* Organisiert die Übertragung von Datenframes zwischen unmittelbar benachbarten Netzwerkknoten und gliedert sich nach IEEE 802 in zwei Teilschichten: die #acr-emph("LLC")-Teilschicht sowie die darunterliegende #acr-emph("MAC")-Teilschicht, welche u. a. die Adressierung über #acr("MAC")-Adressen sowie den Zugriff auf das Übertragungsmedium regelt. #acr("TSN") und #acr("gPTP") setzen ausschließlich auf dieser Schicht auf (siehe @tsn-intro), wodurch sie unabhängig von höheren, routingfähigen Protokollen wie #acr-emph("IP") funktionieren.

#note[TODO: Prüfen, ob eine kurze Abgrenzung zum TCP/IP-Referenzmodell sinnvoll ist, da Zephyr dieses für die höheren Schichten des Netzwerkstacks verwendet (siehe @zephyr-network-arch).]

== Time-Sensitive Networking <tsn-intro>
Auf der in @osi-model eingeführten Sicherungsschicht (Layer 2) trifft Standard-Ethernet keine Aussage über garantierte Übertragungszeiten und arbeitet stattdessen nach dem Best-Effort-Prinzip@ethernetDefinitiveGuide. Switches puffern Pakete in Warteschlangen (Queues) und verwerfen sie bei Überlast. Dies kann in industriellen Anwendungen zu unvorhersehbaren Verzögerungen (Jitter) und Paketverlusten führen. Für zeitkritische Steuerungsanwendungen ist jedoch ein deterministisches Zeitverhalten zwingend erforderlich, bei dem die maximale Übertragungsdauer (Bounded Latency) garantiert ist@zhang2024tsn. Genau hier setzt #acr("TSN") an.
...
#note[TODO: Konkreten Bezug zur eigenen Implementierung herstellen -- welche(r) Thread(s) aus Kapitel 4 (z. B. der bridge-interne Synchronisierungs-Task) laufen kooperativ bzw. präemptiv, und warum wurde diese Entscheidung getroffen?]
...
Ein Treiber bündelt in Zephyr Konfiguration (z. B. Registeradressen, üblicherweise aus dem Device Tree generiert), Laufzeitzustand und eine API mit Funktionszeigern auf die eigentlichen Treiberoperationen (z. B. Senden, Empfangen) @zephyr_drivers. Diese Trennung erlaubt es, dieselbe Treiberimplementierung für mehrere Hardware-Instanzen wiederzuverwenden, die sich nur in Konfiguration und Zustand unterscheiden -- relevant etwa für die beiden ENET-Instanzen `enet` und `enet1g`, die in Kapitel 4 unabhängig voneinander angesprochen werden. Höhere Schichten des Netzwerkstacks kommunizieren dabei nie direkt mit der Hardware, sondern ausschließlich über diese treiberseitige API @zephyr_net_l2.

In der Praxis läuft diese Kommunikation über zwei Einstiegspunkte: Beim Senden reicht der Stack ein fertiges Paket an die Sende-Funktion des Treibers durch, der es an die Hardware übergibt; beim Empfang übernimmt der Treiber ein eingetroffenes Frame von der Hardware und reicht es zur weiteren Verarbeitung an den Stack weiter @zephyr_net_l2. Genau an diesen beiden Punkten -- unmittelbar vor dem physischen Versand bzw. direkt nach dem physischen Empfang -- setzt auch die in @hardware-basics beschriebene Hardware-Zeitstempelerfassung an, da der Treiber die einzige Softwarekomponente ist, die so nah am eigentlichen Signal ausgeführt wird.

#note[TODO: Eigene Treiberanpassungen aus Kapitel 4 hier konkret verorten -- z. B. an welcher Stelle im Treiber die Zeitstempel der zweiten Instanz abgegriffen werden.]

=== Datenfluss im Netzwerkstack
Der *Datenfluss* im Stack lässt sich am besten anhand des Weges eines Pakets nachvollziehen, wobei die zuvor beschriebenen Komponenten der Treiberschicht sowie die in @zephyr-network-arch genannten #acrpl-emph("API") zum Einsatz kommen. Der *Ingress-Pfad* (Empfang) beginnt, wenn ein Ethernet-Frame von der Hardware empfangen wird. Wie in @hardware-basics beschrieben, kann hier durch den #acr("MAC")-Controller ein Hardware-Zeitstempel erfasst werden. Der Zephyr-*Ethernet-Treiber* nimmt diesen Frame samt Zeitstempel-Metadaten entgegen, verpackt ihn -- wie im vorherigen Abschnitt beschrieben -- in einen `net_buf` und reiht ihn in eine zentrale Verarbeitungswarteschlange ein. An dieser Stelle im Software-Stack sind nun alle Informationen verfügbar, welche die #acr("gPTP")-Logik zur Berechnung des `ingress timestamps (t2)` benötigt.
