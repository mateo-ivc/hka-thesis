...
Welches Gerät welche dieser Rollen einnimmt, ist nicht statisch festgelegt, sondern wird im Normalbetrieb dynamisch durch den #acr-emph("BTCA") bestimmt: Alle Geräte einer Domäne tauschen dazu Announce-Nachrichten mit vergleichbaren Qualitätsattributen ihrer Clock aus (u. a. `priority1`, Clock-Klasse und -Genauigkeit) @ieee8021as2025[8.6] und einigen sich verteilt darauf, welche Clock die beste verfügbare Referenz darstellt – diese wird zur Grandmaster Clock. Auf Basis dieses Vergleichs entscheidet jedes Gerät zudem selbst, in welchen Zustand die einzelnen Ports seiner Schnittstellen versetzt werden@ieee8021as2025[10.3.1.2]. In dem in Abschnitt 3.2 beschriebenen Testaufbau dieser Arbeit ist der #acr("BTCA") jedoch nicht aktiv: Da hier ausschließlich die Bridge-Funktionalität selbst validiert werden soll, werden Grandmaster- und Bridge-Rollen den Geräten stattdessen fest zugewiesen.
...
1. Sync-Nachricht: Der Master sendet eine Sync-Nachricht an den Slave. Dabei werden der Sendezeitpunkt ($t_{s}$) auf Master-Seite und der Empfangszeitpunkt ($t_{r}$) auf Slave-Seite erfasst@ieee8021as2025[11.1.3].
...
Die Architektur des Zephyr-Netzwerkstacks ist auf hohe Performance und Flexibilität ausgelegt, was durch mehrere Kernprinzipien erreicht wird. Ein zentrales Prinzip ist das „Zero-Copy“-Verfahren mittels sogenannter Netzwerk-Puffer (`net_buf`) @zephyr_net_buf. Anstatt Datenpakete beim Durchlauf durch die Netzwerkschichten im Speicher zu kopieren, wird lediglich ein Zeiger auf den Puffer weitergereicht. Dies reduziert die #acr("CPU")-Last gegenüber einem kopierenden Ansatz.
