...
#figure(
  table(
    ...
    tab-d[$41,1"s"$], tab-d[$1005$], tab-d[$84 thin 101$], tab-d[$72,4$], tab-d[$28,5$], tab-d[#req("A8") erfüllt],
    table.hline(),
  ),
...
Die Fehlerbetrachtung ergänzt dieses Ergebnis um eine quantitative Einordnung, denn der gesamte Beitrag der bridge-internen Synchronisation bleibt mit rund $11%$ des zulässigen Budgets deutlich unterhalb der Grenze und wird dabei vom Phasen-, nicht vom Frequenzfehler bestimmt. Der Frequenzanteil fällt mit $0,15"ns"$ gegenüber dem Phasenfehler von bis zu $113"ns"$ nicht ins Gewicht.

Ein Befund ist über den reinen Nachweis hinaus festzuhalten. Die Syntonisierung des Slave-Ports verharrt über rund zwei Minuten auf einem konstanten Frequenzversatz von $3,8$#acr("ppm"), bevor sie endgültig einrastet. Hier bleibt das folgenlos, da die Auswertung erst danach beginnt und die #acr-emph("E2E")-Genauigkeit bereits deutlich früher im Toleranzband liegt. Damit ist die Grundlage gelegt, auf der die nachfolgenden Tests aufbauen.


== Validierung über mehrere Hops <mehrhop-validierung>
Die Basisvalidierung hat gezeigt, dass eine einzelne Bridge die geforderten Genauigkeiten einhält. Die geforderte #acr-emph("E2E")-Synchronisationsgenauigkeit von $<=1mu s$ bezieht sich allerdings auf eine Kette von bis zu sieben Hops. Jede zusätzliche Bridge kann dabei eigene Fehlerbeiträge mit sich bringen: die Ungenauigkeit ihrer `residenceTime`-Messung, die Restabweichung der bridge-internen Synchronisation (@bridge-sync-impl) sowie die in @Ungenauigkeiten beschriebene #acr-emph("PHY")-Asymmetrie. Ob sich diese Beiträge über mehrere Hops hinweg unkritisch verhalten oder sich systematisch aufsummieren, lässt sich an einer Einzelbridge nicht beantworten.
...
Analog zur Basisvalidierung werden je Ausbaustufe zunächst der zeitliche Offsetverlauf und anschließend die Verteilung im eingeschwungenen Zustand betrachtet. Die 1. Stufe mit nur einer Bridge wurde bereits in @basisvalidierung ausgewertet. Die folgenden beiden Abschnitte ergänzen Stufe 2 und Stufe 3. Kanal 2 und 3 des Oszilloskops liegen dabei jeweils an Slave- und Master-Port der letzten Bridge in der Kette an, Kanal 4 durchgehend am Endpoint.
...
Für die drei tatsächlich aufgebauten Stufen ist die Bridge-Funktionalität im Sinne von Annex B.3 damit nachgewiesen. Eine Aussage über eine vollständige Kette von sieben Hops lässt sich daraus jedoch nicht ableiten. Sieben Hops entsprechen sechs zwischengeschalteten Bridges, also drei mehr als hier aufgebaut. Linear hochgerechnet ergibt der beobachtete Zuwachs von rund $150 n s$ pro Bridge einen Offset von etwa $880 n s$. Das entspricht bereits rund $88 %$ des zulässigen Toleranzbands. Diese Hochrechnung ist bewusst als grobe Abschätzung und nicht als exakte Prognose zu verstehen. Sie geht von einem konstanten Zuwachs pro Bridge aus -- einer Annahme, die durch die bisher vorliegenden drei Stufen nicht sicher belegt ist.
Dennoch reicht diese Abschätzung aus, um zwei wichtige Erkenntnisse festzuhalten:

- Fehlende Reserve: Die geforderte Grenze von $1mu s$ würde bei voller Kettenlänge nicht mit einem komfortablen Abstand, sondern allenfalls sehr knapp eingehalten werden.
...
Konfiguration A wurde über $1000"s"$ mit $1"kB"$ großen Paketen und einem stufenförmigen Lastprofil durchgeführt. Beginnend bei $5"Mbit/s"$ wurde die angebotene Rate alle $300"s"$ um weitere $5"Mbit/s"$ bis auf $15"Mbit/s"$ angehoben. In der dritten Stufe erreichte der Sender die Sollrate nicht mehr und kam auf effektiv $13,33"Mbit/s"$. Nach $900"s"$ endete die Last. @tab-netzwerklast-durchsatz fasst die je Laststufe protokollierten Durchsatz- und Verlustkennzahlen zusammen. @fig-pps-netzwerklast-ohne-cbs zeigt denselben Lauf als zeitlichen Verlauf.
...
#figure(
  table(
    ...
    table.hline(stroke: 0.2pt + luma(80)),
    tab-d[Angebotene Rate (Sollwert)], tab-d[5,00 Mbit/s], tab-d[10,00 Mbit/s], tab-d[15,00 Mbit/s],
    table.hline(stroke: 0.2pt + luma(80)),

    tab-d[Effektive Senderate], tab-d[5,00 Mbit/s], tab-d[10,00 Mbit/s], tab-d[13,33 Mbit/s],
    table.hline(stroke: 0.2pt + luma(80)),
    tab-d[Laufzeit],
...
Mit dem Übergang in die dritte Laststufe bei $t approx 610"s"$ ändert sich das Bild schlagartig. Am Port von Bridge 1 kommen nun rund $14"Mbit/s"$ an, von denen die Bridge einen erheblichen Teil verwirft. Der Drop-Anteil schwankt für den Rest der Stufe zwischen etwa $30$ und annähernd $100"%"$ der jeweils anliegenden Rate. Die in @tab-netzwerklast-durchsatz ausgewiesenen $20,15"%"$ beziehen sich demgegenüber auf die gesamte Stufe 3 und damit auf einen Zeitraum, der auch die Sekunden vor dem Einbruch einschließt. Zeitgleich verlässt der Offset aller drei Messpunkte sein bis dahin stabiles Band. Der zeitliche Zusammenfall von einsetzendem Paketverlust und ausbrechendem Offset ist der zentrale Befund dieses Laufs.
...
Die Gegenprobe bestätigt die Ursache. Nach dem Abschalten der Last bei $t approx 910"s"$ liefert die Messung ab $t approx 960"s"$ wieder gültige Flanken, und alle drei Messpunkte kehren in ihr ursprüngliches Band zurück, in dem sie bis zum Ende der Messung verbleiben.
...
In Konfiguration B bleibt `alloc_fail_ptp` über sämtliche Messpunkte des Laufs bei null, während im selben Zeitraum über $1,4$ Millionen Allokationsversuche des Best-Effort-Verkehrs fehlschlagen. Kein einziges #acr("gPTP")-Frame musste über die gesamte Messdauer mangels Puffer verworfen werden. Damit ist die in @nachweis-pufferversorgung festgelegte Bedingung erfüllt, und zwar unabhängig von der #acr("PPS")-Messung.

=== Vergleich und Einordnung
Der gemessene Unterschied ist dabei nicht dem #acr("CBS") zuzuschreiben. Der Shaper regelt ausschließlich die Senderichtung, die Last trifft die Bridge hier aber am Eingang. Ausschlaggebend sind deshalb die empfangsseitigen Maßnahmen aus @impl-lastschutz, insbesondere der reservierte Pufferpool. Ohne ihn teilen sich Nutzlast und #acr("gPTP")-Nachrichten denselben Speicher, und die Bridge verwirft unter Last beide gleichermaßen. Die Zählerauswertung in @ergebnisse-pufferversorgung belegt diesen Mechanismus direkt, denn in Konfiguration A traten ausweislich der Logs Allokationsfehler auch auf der vollständig unbelasteten Instanz auf, über die ausschließlich der #acr("gPTP")-Verkehr läuft. Da die Maßnahmen aus @tab-lastschutz-massnahmen erst in ihrer Kombination greifen, sind sie bewusst nicht einzeln bewertet worden.

Damit ist erreicht, was eine Time-Aware Bridge leisten muss. Gefordert ist nicht, dass der Nutzdatendurchsatz unter Last erhalten bleibt, sondern dass die Zeitsynchronisation es tut. In Konfiguration B verwirft die Bridge bei $42"Mbit/s"$ rund die Hälfte des eingehenden Nutzverkehrs, aber kein einziges #acr("gPTP")-Frame, und der Offset bleibt unverändert innerhalb der geforderten Grenzen. Die Degradation trifft ausschließlich die unkritische Verkehrsklasse, während in Konfiguration A beide betroffen sind und damit genau die Funktion ausfällt, die den Zweck der Bridge ausmacht.
