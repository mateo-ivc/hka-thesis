...
Nachzuweisen sind dabei zwei getrennte Größen, die im selben Lauf erfasst werden. Für #req("A3") ist es die vom #acr("gPTP")-Stack je Port aus dem _pDelay_-Mechanismus geschätzte `neighborRateRatio`, für #req("A8") die vom bridge-internen Servo aufgeprägte Ratenkorrektur. Beide sind fraktionale Frequenzabweichungen und damit gegen denselben Grenzwert prüfbar, unterscheiden sich inhaltlich aber grundlegend: Die erste ist eine Messung, die zweite eine Stellgröße.
...
Das Mittelungsintervall $tau_0$ wird nicht angenommen, sondern aus den Zeitstempeln des Logs selbst bestimmt, da es sich aus der Aktualisierungsrate der jeweiligen Größe ergibt und nicht frei wählbar ist. Für den Servo folgt es aus den #acr("PPS")-Flanken, für die `neighborRateRatio` aus dem _pDelay_-Intervall. Beide liegen bei $tau_0 approx 1"s"$ und damit auf dem Intervall, aus dem der Standard den Grenzwert selbst ableitet: Die $0,1$ #acr("ppm") entsprechen einer Messung über das voreingestellte _pDelay_-Intervall von $1"s"$ bei einer Granularität von $40n s$ @ieee8021as2025[B.2.4]. #req("A2") und #req("A3") sind damit nicht unabhängig, sondern gehen auseinander hervor.
...
Der _pDelay_-Mechanismus aus @pDelay-mechanism setzt voraus, dass die Signallaufzeit zwischen zwei Ports in beide Richtungen symmetrisch ist @ieee8021as2025[11.2]. Nur unter dieser Annahme lässt sich aus der Summe $t_4 - t_1$ und $t_3 - t_2$ ein einseitiger `meanLinkDelay` berechnen. Der vorliegende Testaufbau verletzt jedoch diese Annahme. Wie in @testaufbau beschrieben, verfügt der 1-Gbit/s-#acr("PHY") über eine #acr("SFD")-Erkennung. Die beiden 10/100-Mbit/s-#acrpl("PHY") besitzen diese Funktion jedoch nicht. Alles, was in einem Ethernet-Frame vor dem #acr("SFD") steht, geht dadurch in die gemessene Zeitspanne ein. Zusätzlich ist diese interne Verarbeitungszeit zwischen Sende- und Empfangsrichtung nicht notwendigerweise gleich groß, wodurch die Symmetrieannahme des _pDelay_-Mechanismus zusätzlich verletzt wird. Diese Asymmetrie erklärt, warum an Hops mit einem 10/100-Mbit/s-#acr("PHY") ein deutlich höherer `meanLinkDelay` gemessen wird.
