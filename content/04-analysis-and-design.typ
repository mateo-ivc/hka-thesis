...
Werden diese Anforderungen eingehalten, gilt die eigentliche Zielgröße dieser Arbeit: Zwei PTP-Instanzen, die höchstens sieben Hops voneinander entfernt sind, sind auf $1mu s$ peak-to-peak genau synchronisiert @ieee8021as2025[B.3].

Die Grenze gilt zudem nur "during steady-state operation". Der Standard definiert diesen Zustand als denjenigen, in dem jede Instanz in jedem Sync-Intervall Zeitinformation empfängt @ieee8021as2025[B.3]; daraus folgt unmittelbar die in @anforderung-netzwerklast behandelte Robustheitsanforderung. Davon unabhängig wertet diese Arbeit erst nach dem Einschwingen der Regelung aus. Das ist eine eigene Festlegung und wird in @statistische-auswertung beschrieben.
...
Für die vorliegende Arbeit ist diese Anforderung trotzdem relevant, weil die untersuchte Implementierung die Weiterleitung rein in Software durchführt.
...
#figure(
  table(
    ...
    tab-d[normativ, Empfehlung @ieee8021as2025[B.2.2]],
    table.hline(stroke: 0.2pt + luma(80)),
