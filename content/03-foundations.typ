...
Der Name des #acr("BTCA") spiegelt dabei bereits eine Umbenennung wider, die der Standard mit seiner aktuellen Ausgabe durchgängig vollzogen hat. An die Stelle der Rollenbezeichnungen _Master_ und _Slave_ sind _timeTransmitter_ und _timeReceiver_ getreten. Die vorliegende Arbeit verwendet allerdings weiterhin die älteren Begriffe, da die untersuchte Zephyr-Implementierung sie in Bezeichnern, Konfigurationsoptionen und Logausgaben durchgängig führt und eine abweichende Benennung im Text die Zuordnung zwischen Beschreibung und Quellcode erschweren würde. Aus demselben Grund wird die über den pDelay-Mechanismus gemessene `neighborRateRatio` weiterhin so bezeichnet, obwohl der Standard sie in seiner aktuellen Ausgabe als `nrrPdelay` führt @ieee8021as2025[11.2.13.14].
