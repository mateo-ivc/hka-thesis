...
- *#acr-emph("GM"):* Die #acr("GM") bildet die oberste Zeitreferenz für die gesamte Domäne. Sie verfügt in der Regel über eine hochpräzise Zeitquelle und verteilt ihre Zeit im Netzwerk@ieee8021as2025[3.15].
...
Der Name des #acr("BTCA") spiegelt dabei bereits eine Umbenennung wider, die der Standard mit seiner aktuellen Ausgabe durchgängig vollzogen hat. An die Stelle der Rollenbezeichnungen _Master_ und _Slave_ sind _timeTransmitter_ und _timeReceiver_ getreten. Die vorliegende Arbeit verwendet allerdings weiterhin die älteren Begriffe, da die untersuchte Zephyr-Implementierung sie in Bezeichnern, Konfigurationsoptionen und Logausgaben durchgängig verwendet und eine abweichende Benennung im Text die Zuordnung zwischen Beschreibung und Quellcode erschweren würde. Aus demselben Grund wird die über den _pDelay_-Mechanismus gemessene `neighborRateRatio` weiterhin so bezeichnet, obwohl der Standard sie in seiner aktuellen Ausgabe als `nrrPdelay` führt @ieee8021as2025[11.2.13.14].
...
Bei dem in @sync-mechanism (linke Seite) dargestellten Two-Step-Verfahren erfolgt der Austausch in zwei Schritten:

1. Sync-Nachricht: Der Master sendet eine Sync-Nachricht an den Slave. Dabei wird der Sendezeitpunkt ($t_s$) auf Master-Seite und der Empfangszeitpunkt ($t_r$) auf Slave-Seite erfasst@ieee8021as2025[11.1.3].

2. #emph[Follow\_Up]-Nachricht: Um dem Slave die notwendigen Informationen für die Synchronisation bereitzustellen, sendet der Master anschließend eine #emph[Follow\_Up]-Nachricht. Diese enthält den präzisen Sendezeitpunkt der Sync-Nachricht (#gls("preciseorigintimestamp")[`preciseOriginTimestamp`]), das #gls("correctionfield")[`correctionField`] sowie die `rateRatio` @ieee8021as2025[11.1.3].
  ...
  - *`correctionField`:* Ein von der Grandmaster Clock mit dem Wert "0" initialisierter Korrekturwert, der auf dem Weg zum Slave alle Verzögerungen aufsummiert, die im `preciseOriginTimestamp` noch nicht enthalten sind. Dazu zählen sowohl die gemessene #gls("path-delay")[Leitungsverzögerung] als auch die #gls("residence-time")[`residenceTime`], also die Zeit, die eine Nachricht innerhalb jeder zwischengeschalteten Bridge verbringt. Damit lässt sich der tatsächliche Sendezeitpunkt der Grandmaster Clock trotz dieser Verzögerungen exakt rekonstruieren@ieee8021as2025[10.2.2].

  - *#gls("rateratio")[`rateRatio`]:* Das Verhältnis der Frequenz der Grandmaster Clock zur Frequenz der lokalen Clock des Slaves. Ausgehend vom Wert eins bei der Grandmaster Clock wird sie über jeden Hop hinweg fortgeschrieben und beschreibt so stets das aktuelle Frequenzverhältnis zur ursprünglichen Zeitquelle (siehe @gptp-bridge). Neben der Offset-Korrektur erlaubt sie dem Slave dadurch auch, seine lokale Taktrate an die des Masters anzupassen@ieee8021as2025[10.2.2].
...
Ein weiterer wichtiger Mechanismus ist das Berechnen des #gls("path-delay")[`propagationDelay`]. Hierbei wird ermittelt, wie lange ein #gls("frame")[Frame] auf der physischen Leitung benötigt.

#figure(
  image("../assets/Sync/gPTP-pDelay-mechanism.png", width: 80%),
  caption: [Darstellung des _pDelay_-Mechanismus#cap-long-only[, in Anlehnung an @ieee8021as2025]],
) <pDelay-mechanism>

Der Mechanismus nutzt drei Arten von Nachrichten.
Der Initiator sendet zuerst eine `pDelay_Req`-Nachricht. Dabei wird beim Senden der Nachricht der Zeitstempel $t_1$ und beim Empfangen durch den Partner der Zeitstempel $t_2$ aufgenommen. Anschließend sendet der Empfänger die Nachricht `pDelay_Resp` zurück, wobei die Zeitstempel $t_3$ (beim Senden) und $t_4$ (beim Empfangen) erfasst werden. Mit der Nachricht `pDelay_Resp_Follow_Up` wird zuletzt der Zeitstempel $t_3$ an den Initiator übermittelt@ieee8021as2025[11.1.2].
...
Wie in @pdelay-calc dargestellt, entspricht $D$ dem durchschnittlichen `propagationDelay`. Neben $D$ liefert der _pDelay_-Mechanismus über die Zeit hinweg noch eine weitere Größe: Aus der Steigung aufeinanderfolgender $t_("ir")$- bzw. $t_("ri")$-Messreihen lässt sich die #gls("neighborrateratio")[`neighborRateRatio`] ableiten, also das Frequenzverhältnis zum unmittelbaren Nachbarn @ieee8021as2025[11.2.19.3.3]. Sie beschreibt, anders als die über #emph[Follow\_Up]-Nachrichten propagierte `rateRatio`, das Frequenzverhältnis ausschließlich zwischen zwei direkt benachbarten Ports @ieee8021as2025[10.2.5.7], das von einer Bridge zur Fortschreibung der eigenen `rateRatio` benötigt wird @ieee8021as2025[10.2.8].
...
2. *Messung der `residenceTime`:* Bevor die Bridge die Nachricht weiterleiten kann, durchläuft diese intern den Netzwerk-Stack des Geräts. Die dafür benötigte Zeitspanne bis zum Absenden auf dem Master-Port zum Zeitpunkt $t_s$ wird als `residenceTime` bezeichnet und ergibt sich aus $t_s - t_r$.

3. *Aktualisierung der `rateRatio`:* Die Bridge verknüpft die im #emph[Follow\_Up] empfangene `rateRatio` mit der über den in @pDelay-mechanism beschriebenen _pDelay_-Mechanismus lokal gemessenen `neighborRateRatio`, die dem Frequenzverhältnis zur Master-Clock entspricht: $ "rateRatio"_("neu") = "rateRatio"_("alt") dot "neighborRateRatio" $
  Dadurch bleibt die `rateRatio` über beliebig viele Hops hinweg gültig und beschreibt stets das Frequenzverhältnis zwischen der Grandmaster Clock und der lokalen Clock der letzten Bridge@ieee8021as2025[11.1.3].

4. *Aktualisierung des `correctionField`:* Zum eingehenden `correctionField` addiert die Bridge sowohl die gemessene Leitungsverzögerung $D$, die mit der eingehenden `rateRatio` skaliert wird, als auch die zuvor ermittelte `residenceTime`, welche mit der soeben aktualisierten `rateRatio` skaliert wird@ieee8021as2025[11.1.3]:
$
  "correctionField"_("neu") = "correctionField"_("alt") + D dot "rateRatio"_("alt") + (t_s - t_r) dot "rateRatio"_("neu")
$
Der `preciseOriginTimestamp` bleibt dabei unverändert, da er stets die ursprüngliche Sendezeit der Grandmaster Clock referenziert. Das `correctionField` trägt hingegen die seit dem Grandmaster akkumulierte Korrektur aus Laufzeit und Verarbeitungszeit. Da $D$ und die `residenceTime` jeweils mit der `rateRatio` skaliert werden, liegt das `correctionField` durchgehend in der Zeitbasis der Grandmaster Clock vor, unabhängig davon, wie viele Hops die Nachricht bereits durchlaufen hat.
...
Dabei kann die #acr("PTP")-Clock über die `rate_adjust()`-Funktion die Rate zur Laufzeit anpassen. Die in @bridge-sync-impl beschriebene Implementierung nutzt diesen Mechanismus.

== Hardware-Grundlagen <hardware-basics>
Die in den vorangegangenen Abschnitten beschriebenen Synchronisations- und Messmechanismen setzen voraus, dass die beteiligten #gls("timestamp")[Zeitstempel] mit einer dem geforderten Sub-Mikrosekundenbereich entsprechenden Genauigkeit erfasst werden. Dieser Abschnitt beleuchtet die dafür notwendigen Hardware-Grundlagen. Zunächst wird die #acr-emph("MAC")-Schicht als die Komponente eingeführt, in der sowohl das Timestamping als auch die im Anschluss vorgestellte Verkehrsformung stattfinden. Anschließend wird erläutert, warum reines Software-Timestamping für #acr("gPTP") ungeeignet ist und stattdessen spezialisierte Netzwerk-Hardware benötigt wird. Daran schließt sich die Unterscheidung zwischen #acr("PHY")- und #acr("MAC")-Timestamping als zwei möglichen Realisierungsvarianten dieses Hardware-Timestampings an. Abschließend wird mit dem Credit-Based Shaping der in @tsn-intro angekündigte #acr-emph("AVB")-Mechanismus vorgestellt, mit dem die #acr("MAC")-Schicht die geforderte Bounded Latency durchsetzt.

=== Die MAC-Schicht <mac-layer>
Die #acr("MAC")-Schicht bildet die untere der beiden in @osi-model eingeführten Teilschichten von Layer 2 und übernimmt drei zentrale Aufgaben: das Bilden und Zerlegen von Ethernet-Frames (u. a. Präambel, #acr-emph("SFD"), Header mit Quell- und Ziel-#acr("MAC")-Adresse, Nutzdaten und Prüfsumme), die Adressierung über #acr("MAC")-Adressen sowie die Steuerung des Zugriffs auf das Übertragungsmedium @ethernetDefinitiveGuide. Diese Zugriffssteuerung war ursprünglich als CSMA/CD (Carrier Sense Multiple Access with Collision Detection) notwendig, um auf einem gemeinsam genutzten Bus Kollisionen zwischen gleichzeitig sendenden Teilnehmern zu erkennen und aufzulösen @ethernetDefinitiveGuide. In den heute üblichen geswitchten Vollduplex-Topologien entfällt diese Koordinierung, da jede Verbindung nur noch zwei Teilnehmer besitzt. Framing und Adressierung bleiben jedoch weiterhin Aufgabe der #acr("MAC")-Schicht. Zur Hardware hin ist sie über eine standardisierte Schnittstelle an den #acr("PHY") angebunden, etwa das #acr-emph("MII") oder dessen pinreduzierte Ausprägungen #acr-emph("RMII") und #acr-emph("RGMII"). Welche davon zum Einsatz kommt, hängt von Datenrate und verfügbarer Pinzahl ab @st_mii_rmii. Über diese Schnittstelle wird auch der in @phy-mac-timestamping beschriebene #acr("SFD")-Impuls übertragen.
...
Diese Verzögerungen summieren sich auf, sodass Abweichungen im Bereich von mehreren zehn bis hundert Mikrosekunden entstehen und damit um ein Vielfaches von der durch #acr("gPTP") geforderten Genauigkeit abweichen. Reines Software-Timestamping ist damit für #acr("gPTP") ungeeignet.\
...
Ein für die Zeitstempel-Erfassung besonders relevantes Signal ist dabei der #acr-emph("SFD"): ein festes 1-Byte-Muster (`0xD5`), das im Ethernet-Frame unmittelbar auf die Präambel folgt und das Ende der Präambel sowie den Beginn der eigentlichen Nutzdaten markiert @ethernetDefinitiveGuide[4.]. Da seine Bitfolge im Gegensatz zur Präambel eindeutig ist, lässt sie sich zuverlässig per Logik in Hardware erkennen und eignet sich damit als präziser, reproduzierbarer Auslösepunkt für einen Zeitstempel.

=== PHY- vs. MAC-Timestamping <phy-mac-timestamping>
Für die im vorherigen Abschnitt beschriebenen Mechanismen, insbesondere die Messung der Leitungsverzögerung sowie der `residenceTime`, ist die Genauigkeit der Zeitstempel $t_1$ bis $t_4$ bzw. $t_r$ und $t_s$ entscheidend. Ein Hardware-Zeitstempel kann dabei durch drei unterschiedliche Methoden entstehen, die sich sowohl im Ort der eigentlichen Erfassung als auch im auslösenden Ereignis unterscheiden:
...
- *#acr("MAC")-Timestamping:* Steht kein solcher #acr("SFD")-Impuls zur Verfügung, muss der #acr("MAC") den Beginn eines Frames selbst aus dem Datenstrom erkennen, den der #acr("PHY") über die in @mac-layer genannte Schnittstelle anlegt, und den Zeitstempel aus diesem ableiten. Zwischen der physischen Ankunft des Signals auf der Leitung und der Zeitstempel-Erfassung liegen damit sowohl die Verarbeitung im #acr("PHY") als auch die Übertragung über die Schnittstelle. Diese zusätzliche, von der Schnittstelle abhängige Latenz ist nicht zwangsläufig konstant. Dies liegt daran, dass #acr("MAC") und #acr("PHY") in der Regel unabhängige Taktquellen besitzen. Dadurch muss das Interface-Signal beim Übergang in die Taktdomäne des #acr("MAC") synchronisiert werden, was zusätzlichen Jitter erzeugt @ti_snla242.
...
Die in @mac-layer vorgestellte #acr("MAC")-Schicht verwaltet mehrere interne, priorisierte Sende-Warteschlangen, muss aber entscheiden, in welcher Reihenfolge sie deren Frames auf die eine physische Verbindung legt. Der #acr-emph("CBS") ist einer von mehreren Algorithmen, mit dem #acr("TSN") diese Einplanung für reservierte Verkehrsklassen regelt: Eine priorisierte Warteschlange wird gegenüber Best-Effort-Verkehr bevorzugt bedient, sodass reservierter Verkehr nicht hinter unpriorisiertem Verkehr zurückstehen muss. Damit diese Bevorzugung nicht dazu führt, dass eine priorisierte Warteschlange den Link dauerhaft belegt, begrenzt der #acr("CBS") ihren Sendeanteil auf die für sie vorab reservierte Bandbreite @ieee8021qav2009.

Welcher Warteschlange ein Frame zugeordnet wird, entscheidet seine Priorität, die das Betriebssystem beim Senden festlegt, für lokal erzeugte ebenso wie für weitergeleitete Frames. Übertragen wird sie im 3 Bit breiten #acr-emph("PCP")-Feld des #acr-emph("VLAN")-Tags, das die Verkehrsklasse des Frames kennzeichnet @ieee8021qav2009. Anhand dieses Werts leitet die #acr("MAC")-Schicht das Frame in die passende, von einem eigenen Credit-Zähler gesteuerte Warteschlange ein und legt damit fest, welchem #acr("CBS") es unterliegt.

Jeder vom #acr("CBS") verwalteten Warteschlange ist dabei ein Credit-Zähler zugeordnet @ieee8021qav2009. Ist die Warteschlange nicht leer, sendet aber gerade nicht (weil z. B. eine andere Warteschlange den Link belegt), steigt der Credit mit der Rate `idleSlope`, welche der für diese Klasse reservierten Bandbreite entspricht. Während die Warteschlange aktiv sendet, sinkt der Credit mit der Rate `sendSlope`, die sich aus `idleSlope` abzüglich der Port-Datenrate ergibt und daher negativ ist. Ein Frame darf nur dann zur Übertragung ausgewählt werden, wenn der Credit größer oder gleich null ist. Sinkt er durch das Senden unter null, muss die Warteschlange warten, bis er sich durch erneutes Ansammeln wieder erholt hat. Über die Zeit gemittelt kann eine Warteschlange dadurch nicht mehr als ihre reservierte Bandbreite `idleSlope` in Anspruch nehmen, wodurch sich die maximale Wartezeit eines Frames in dieser Warteschlange bestimmen lässt @zhang2024tsn.
...
Die #acr("MAC")-Peripherie des in dieser Arbeit eingesetzten #acr-emph("SoC") stellt einen solchen kreditgeformten Sende-Ring bereit; wie er im Testaufbau angebunden und konfiguriert wird, beschreibt @impl-lastschutz.

== Das Echtzeitbetriebssystem Zephyr

Nachdem die konzeptionellen Grundlagen des #acr("gPTP") sowie die Anforderungen an die Hardware für präzises Timestamping erläutert wurden, stellt dieser Abschnitt das Echtzeitbetriebssystem Zephyr vor. Es dient als Software-Plattform, in der die abstrakte Protokolllogik und die Hardware-Fähigkeiten zusammengeführt werden, um eine funktionale Einheit zu bilden. Ein #acr-emph("RTOS") ist speziell dafür entwickelt, Aufgaben innerhalb eines fest definierten Zeitrahmens auszuführen. Im Gegensatz zu Universalbetriebssystemen, die auf maximalen Durchsatz optimiert sind, kommt es beim #acr("RTOS") auf Determinismus an. Das bedeutet, dass das Zeitverhalten absolut vorhersagbar sein muss. Kern ist dabei der Scheduler, der die #acr("CPU")-Zuweisung steuert. Dieser sorgt dafür, dass kritische Prozesse priorisiert werden und so ihre Fristen einhalten können.

Zephyr umfasst mehr als einen Kernel. Es handelt sich um eine modulare Plattform, die von der Linux Foundation verwaltet wird @zephyr_about und Open-Source-Standards in die Embedded-Entwicklung überträgt. Im Gegensatz zu klassischen #acr("RTOS") wie Free#acr("RTOS"), die meist nur einen einfachen Kernel bieten, bringt Zephyr Treiber, Software-Stacks und viele weitere Komponenten mit.
...
- *Netzwerkprotokolle*: #acr("IP"), #acr-emph("UDP"), #acr-emph("TCP")
...
=== Die Treiberschicht
Die im vorherigen Abschnitt genannten #acrpl-emph("API") setzen auf dem allgemeinen Zephyr-Treibermodell auf, das die unterste Schicht des Netzwerkstacks bildet und den eigentlichen Kontakt zur Hardware herstellt. Da dieses Modell und die darunterliegende Schnittstelle zur Ethernet-Hardware für die in @impl-lastschutz beschriebenen Anpassungen am Ethernet-Treiber unmittelbar relevant sind, werden beide im Folgenden kurz umrissen.

Ein Treiber bündelt in Zephyr die Konfiguration (z. B. Registeradressen, üblicherweise aus dem #gls("devicetree")[Device Tree] generiert), den Laufzeitzustand und eine #acr("API") mit Funktionszeigern auf die eigentlichen Treiberoperationen (z. B. Senden, Empfangen) @zephyr_drivers. Diese Trennung erlaubt es, dieselbe Treiberimplementierung für mehrere Hardware-Instanzen wiederzuverwenden, die sich nur in Konfiguration und Zustand unterscheiden. Dies ist für die beiden ENET-Instanzen `enet` und `enet1g` relevant, die in @implementierung unabhängig voneinander angesprochen werden. Höhere Schichten des Netzwerkstacks kommunizieren dabei nie direkt mit der Hardware, sondern ausschließlich über diese treiberseitige #acr("API") @zephyr_net_l2.
...
Beide Einstiegspunkte greifen dabei auf dieselbe Struktur zurück, über die Treiber und Ethernet-Controller Frames austauschen: #gls("descriptor-ring")[Deskriptor-Ringe] im Arbeitsspeicher. Ein Deskriptor beschreibt ein einzelnes Frame über die Adresse seines Datenpuffers, dessen Länge sowie Statusbits, die unter anderem festhalten, ob er gerade der Software oder dem Controller gehört. Eine feste Anzahl solcher Deskriptoren bildet einen zyklisch durchlaufenen Ring, über den beide Seiten als Erzeuger und Verbraucher zusammenarbeiten. Der Controller überträgt die Frame-Daten per #acr("DMA") in die referenzierten Puffer und gibt den Deskriptor anschließend wieder frei, ohne dass die #acr("CPU") an der Übertragung beteiligt ist @nxp_imxrt1170_refman. Sende- und Empfangsrichtung besitzen dabei jeweils eigene Ringe.

Stellt eine #acr("MAC")-Peripherie mehrere Ringe je Richtung bereit, bildet jeder Ring eine eigene Warteschlange. Auf der Sendeseite entsprechen diese den in @cbs beschriebenen Sende-Warteschlangen, deren Einplanung dort dem #acr("CBS") unterliegt @nxp_mcuxpresso_enet. Auf der Empfangsseite kann der Controller eingehende Frames anhand konfigurierbarer Kriterien auf die Ringe verteilen. Da die Ringe softwareseitig bedient werden, entscheidet die Treiberimplementierung darüber, welche von ihnen tatsächlich genutzt werden @zephyr_eth_nxp_enet_source.

=== Datenfluss im Netzwerkstack
Der Datenfluss im Stack lässt sich am besten anhand des Weges eines Pakets nachvollziehen, wobei die zuvor beschriebenen Komponenten der Treiberschicht sowie die in @zephyr-network-arch genannten #acrpl-emph("API") zum Einsatz kommen. Der #gls("ingress-egress")[Ingress-Pfad] (Empfang) beginnt, wenn ein Ethernet-Frame von der Hardware empfangen wird. Wie in @hardware-basics beschrieben, kann hier durch den #acr("MAC")-Controller ein Hardware-Zeitstempel erfasst werden. Der Zephyr-Ethernet-Treiber nimmt diesen Frame samt Zeitstempel-Metadaten entgegen, verpackt ihn in einen `net_buf` und reiht ihn in eine zentrale Verarbeitungswarteschlange ein. An dieser Stelle im Software-Stack sind nun alle Informationen verfügbar, welche die #acr("gPTP")-Logik zur Berechnung des `ingress timestamps` benötigt.

Beim #gls("ingress-egress")[Egress-Pfad] (Senden) erstellt die Applikationslogik einen `net_buf`, der die zu sendende Nachricht enthält. Dieser wird über die Layer-2-#acr("API") an den Treiber übergeben. Nachdem der Treiber das Paket physisch versendet und der #acr("MAC") den exakten Sendezeitpunkt als `egress timestamp` erfasst hat, wird diese Zeitinformation über einen Callback-Mechanismus (`tx_tstamp`) der Timestamping-Schnittstelle an die Applikation zurückgemeldet @zephyr_net_if_source. Dieser Mechanismus ist die softwareseitige Realisierung der Anforderung aus dem #acr("gPTP")-Standard, den exakten Sendezeitpunkt für die Erstellung der #emph[Follow\_Up]-Nachricht zu kennen.
