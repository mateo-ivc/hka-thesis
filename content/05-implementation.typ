...
Für die in @bridge-sync-impl beschriebene bridge-interne Synchronisation sind die #acr("PTP")-Timer sowie deren spezifische Capture- und Compare-Funktionen zwingend erforderlich. Die #acr("PTP")-Timer sind von beginnn an aktiv, die Funktionen jedoch nicht, diese müssen daher entsprechend konfiguriert werden.
...
Um diese Hardware-Events softwareseitig zu verarbeiten, wurde die #acr("ISR") `ptp_clock_nxp_enet_isr()` erweitert. Löst nun eines der beiden Events aus, liest die #acr-emph("ISR") den exakten Hardware-Zeitstempel aus und übergibt ihn über eine Message-Queue an einen Task. Dies schafft die notwendige Datengrundlage, um beide Timer im Anschluss präzise miteinander zu synchronisieren.
...
Um dieses Problem zu lösen, wurden wie in @lst:sync-callback-fix zusehen ist, zwei sich ergänzende Anpassungen im #acr("gPTP")-Stack vorgenommen.
...
Zum anderen würde die die Zustandsmaschine des Sync-Sendepfads ohne die Änderungen in @lst:sync-send-timeout unbegrenzt auf den TX-Zeitstempel warten. Daher wird beim Versenden der Sync-Nachricht ein zusätzlicher Software-Zeitstempel aufgenommen, der die im Zustand `GPTP_SYNC_SEND_SEND_FUP` verstrichene Zeit misst. Bleibt der TX-Zeitstempel länger als 10 ms aus, wird der Callback über `gptp_sync_send_abort()` explizit deregistriert und der Zustand zurück auf `GPTP_SYNC_SEND_SEND_SYNC` gesetzt, um den Sync-Mechanismus gezielt neu zu starten, statt auf einen Zeitstempel zu warten, der möglicherweise nie mehr eintrifft.
...
Die Timeoutzeit von 10 ms wurde bewusst gewählt. Da im Standardfall alle 125 ms eine neue Sync-Nachricht versendet wird, bleibt der Zustandsmaschine damit ausreichend Zeit, um auf den TX-Zeitstempel-Callback zu warten, ohne dass ein hängen gebliebener Callback bis zum nächsten Sync-Intervall unbemerkt bliebe.
...
Die Erweiterung des Treibers ist über `CONFIG_ETH_NXP_ENET_1G_AVB` schaltbar und aktiviert einen zweiten Hardware-Ring (Ring 1) samt zugehöriger #acr-emph("DMA")-Deskriptoren, Sende-Semaphore und Staging-Puffer. Sie greift bewusst nur auf die `enet1g`-Instanz, die diese Hardware-Unterstützung besitzt. Die `enet`-Instanz bleibt davon unberührt.
...
Zephyr kann eingehende Pakete anhand einer im Treiber zugewiesenen Priorität auf mehrere getrennte, priorisierte Empfangs-Warteschlangen zum Netzwerkstack verteilen. Der ENET-Treiber setzte diese Priorität jedoch nie, sodass sämtliche Pakete mit der Standardpriorität 0 ankommen. Dadurch landen alle Pakete unabhängig von der Verkehrsklasse derselben Warteschlange. Die konfigurierten Verkehrsklassen blieben auf der Empfangsseite damit wirkungslos. Behoben wurde dies durch @lst:rx-traffic-class, indem #acr("PTP")-Frames im Treiber vor der Übergabe an den Netzwerkstack die Priorität `NET_PRIORITY_IC` erhalten und damit in einer eigenen Warteschlange landen, dieselbe Klasse, die das #acr("gPTP")-Subsystem sendeseitig ohnehin bereits setzt. `eth_get_ptp_data()` überprüft dabei, ob das das Frame einem #acr("gPTP")-Frame gleicht.
...
Ursächlich dafür war die Reihenfolge im Treiber. Der Netzwerkpuffer allokiert Speicher für ein Paket, bevor das Frame aus dem Ring gelesen wird. Das bedeutet, dass der Treiber beide Verkehrsklassen bei der Allokation gleich behandelt. Weder die Hardware-Klassifizierung noch die zuvor beschriebenen Empfangs-Verkehrsklassen können dadurch beeinflussen, welche Frames einen Puffer erhalten.
