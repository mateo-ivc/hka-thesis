...
Der #acr("PTP")-Timer der Gigabit-Instanz (`enet1g`) wurde so konfiguriert, dass er bei einem Compare-Event einen Puls über einen #acr("GPIO")-Pin ausgibt. Der #acr("PTP")-Timer der 10/100-Mbit-Instanz (`enet`) wurde so konfiguriert, dass er diesen Puls per Capture-Event einliest. Da der #acr("gPTP")-Slave-Port der Bridge nach @testaufbau fest auf `enet1g` liegt, ist dessen Timer bereits durch das #acr("gPTP")-Protokoll selbst korrekt zur Grandmaster Clock synchronisiert und dient deshalb innerhalb der Bridge als Zeitreferenz. Der Timer des #acr("gPTP")-Master-Ports auf `enet` wird vom #acr("gPTP")-Stack hingegen nicht angefasst und muss daher durch die in @bridge-sync-impl beschriebene bridge-interne Synchronisierung an den Timer von `enet1g` angeglichen werden. Diese Zuordnung wirkt auf den ersten Blick vertauscht, da im #acr("gPTP")-Protokoll selbst stets die Slave-Seite korrigiert wird -- hier ist es jedoch, rein bridge-intern, umgekehrt: Der bereits synchronisierte Slave-Port dient als Referenz für den noch unsynchronisierten Master-Port. Zusätzlich wurde die #acr-emph("ISR") erweitert:\
